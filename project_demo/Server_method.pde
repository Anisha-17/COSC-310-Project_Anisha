public void ServerMethod(String message) {
  ...
  if (mes.equals("FIRE")) { //During FIRE special case, all doors turn green indefinitely
    ...
    col_bathroom_2 = g;
    col_conference =g;
    androidInfo("FIRE", "-1", false, ""); //DoorToAccess variable is not relevant for this special case. DATETIME variable will still be calculated in androidInfo
    ...
    col_bathroom_2 = b;
    col_conference =b;
    androidInfo("INTRUDER", "-1", false, ""); //DoorToAccess variable is not relevant for this special case. DATETIME variable will still be calculated in androidInfo
  } else { //<>//
    try{
